...
class Pagination(object):
    ...
    def page_html(self):
        ...
        if self.current_page <= 1:
            prev = '<li><a href="#" class="layui-disabled">上一页</a></li>'
        else:
        ...
        for i in range(pager_start, pager_end + 1):
            ...
            if self.current_page == i:
                ...
                # 新版 layui
                tpl = '<span class="layui-laypage-curr"><em class="layui-laypage-em"></em><em>{pages}</em></span>'.format(pages=i)
            else:
                tpl = '<li><a href="%s?%s">%s</a></li>' % (self.base_url, self.query_params.urlencode(), i,)
            page_list.append(tpl)

        if self.current_page >= self.pager_count:
            nex = '<li><a href="#" class="layui-disabled">下一页</a></li>'
        else:
            self.query_params['page'] = self.current_page + 1
